...
class Form
{
	...
	public function form_field($fld = array())
	{
		...
		if (!empty($fld['disabled'])){
			$output .= ' disabled="'.$fld['disabled'].'"';
		}
	...
	*/ 
	...
	public function build($form) { 
		...
		$output = '<form action="'.$this->action.'" method="'.$this->method.'" id="'.$this->id.'"'.$enctype.' class="'.$this->classes.'">'; 
		$output .= $fields; 
		$output .= '</form>'; 
		return $output; 
	}
	
	public function build_fields($form){
		$output = array(); 
		foreach($form as $key => $fld) { 
			$fld['name'] = $key;
			if ($fld['type'] != 'select' && 
